...
type ColType = "text" | "number" | "select" | "ref" | "computed" | "line";
type Col = {
...
function TargetGrid({ cfg, darkMode, canEdit, tenantId, userId, onToast, allowedExamProcessIds = null }: {
  ...
  // [Line 전환] 제품/파트 컬럼은 기본 숨김(레거시 데이터·identity·Excel 내보내기용으로 값은 보존, 표시만 제거).
  const [hidden, setHidden] = useState<Set<string>>(new Set(["part_name"]));
  ...
  // [Line 전환] 편집 폼에서는 제품/파트 입력 UI 제거(제품군→그룹까지만). formCols 자체는 Excel import·검증에 계속 사용.
  const formRenderCols = useMemo(() => formCols.filter((c) => c.key !== "part_name"), [formCols]);
  ...
  // 선택 변경 → 하위 초기화 + 텍스트 컬럼/group_id 동기화. cascade select 는 "신규 등록"에서만 렌더되므로
  ...
  const saveRow = async () => {
    ...
    try {
      ...
      if (rows.some((r) => r.id !== editRow.id && identityKey(r) === key)) { setError("동일한 연도/제품군/그룹/인증레벨 항목이 이미 있습니다."); setSaving(false); return; }
  ...
  const buildImportPreview = async (file: File) => {
    ...
    try {
      ...
      for (let i = 0; i < raw.length; i++) {
        ...
        if (!row.group_id) {
          const gn = String(row.group_name ?? "").trim(), cn = String(row.product_group ?? "").trim();
          if (gn) {
    ...
    <section className={`rounded-3xl p-5 shadow-sm ring-1 ${darkMode ? "bg-slate-900 ring-slate-700" : "bg-white ring-slate-200"}`}>
      ...
      <div className="mb-2 flex flex-wrap items-center gap-1.5">
        ...
        <span className="ml-auto flex flex-wrap gap-1.5">
          ...
          {canEdit && <button className="rounded-xl bg-slate-900 inline-flex items-center justify-center min-h-[42px] min-w-[72px] px-5 text-sm font-semibold text-white hover:bg-slate-800 whitespace-nowrap" onClick={() => setEditRow({ year: new Date().getFullYear() })}>등록</button>}
        </span>
      </div>
...
function AnnualAutoAggregatePanel({ darkMode, tenantId, refreshKey }: { darkMode: boolean; tenantId: string; refreshKey?: number }) {
  ...
  // [연도 정합] 표시 연도 = 계산 연도 보장(기본 new Date().getFullYear() 가 옵션에 없으면 최신 연도로 정규화).
  const selectedYear = useMemo(() => (opts.years.includes(year) ? year : (opts.years[0] ?? year)), [opts.years, year]);

  // 목표 합계(exam_annual_targets) — 연도 + 그룹/제품/파트/인증단계 필터 매칭.
  const targetTotal = useMemo(() => targets.filter((t) =>
    str(t.year) === selectedYear &&
  ...
  ).reduce((s, t) => s + (Number(t.target_count) || 0), 0), [targets, selectedYear, f]);

  const perf = useMemo(() => calculateAnnualPerformance(records, selectedYear, {
    target: targetTotal,
    filters: {
  ...
  }), [records, selectedYear, f, targetTotal]);
  ...
  return (
    <section className={section}>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-1.5">
        <select value={selectedYear} onChange={(e) => setYear(e.target.value)} className={selCls}>
...
function MonthlyAutoAggregatePanel({ darkMode, tenantId, refreshKey }: { darkMode: boolean; tenantId: string; refreshKey?: number }) {
  ...
  // [연도 정합] 실제 집계 연도는 반드시 옵션에 존재하는 값으로 정규화. 기본값 new Date().getFullYear() 가 데이터에 없으면
  //  controlled select 는 첫 옵션을 "표시"하지만 state 는 존재하지 않는 연도로 남아 화면(예 2025)과 계산(예 2026)이 어긋난다.
  //  → select value 와 집계에 동일한 selectedYear 를 사용해 표시 연도 = 계산 연도를 보장.
  const selectedYear = useMemo(() => (opts.years.includes(year) ? year : (opts.years[0] ?? year)), [opts.years, year]);
  const stageAgg = useMemo(() => calculateMonthlyPerformanceYear(stageRecords, selectedYear, filters), [stageRecords, selectedYear, filters]);
  const equipAgg = useMemo(() => calculateEquipmentMonthlyPerformanceYear(equipRecords, selectedYear, filters), [equipRecords, selectedYear, filters]);
